...
As a test case to evaluate the usefulness of the good-sounds.org framework we setup an experiment to rate the goodness of single notes. The goal of the experiment was to build models from the uploaded sounds and the community annotations with which we can then automatically rate the sound goodness. We then compared the results of these models with the ones we obtained in our previous work using expert annotations.  
...
\begin{figure}
 \centerline{\framebox{
 \includegraphics[width=\columnwidth]{figs/scores_2.png}}}
